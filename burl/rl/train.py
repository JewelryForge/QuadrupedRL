...
from burl.utils import make_cls, timestamp, g_cfg, g_dev
...
class OnPolicyRunner:
    ...
    def learn(self):
        ...
        tot_iter = self.current_learning_iteration + g_cfg.num_iterations

        for it in range(self.current_learning_iteration, tot_iter):
            start = time.time()
            with torch.inference_mode():
                for i in range(g_cfg.num_steps_per_env):
        ...
        self.current_learning_iteration += g_cfg.num_iterations
        self.save(os.path.join(self.log_dir, 'model_{}.pt'.format(self.current_learning_iteration)))

    def log(self, locs, width=80, pad=35):
        self.total_timesteps += g_cfg.num_steps_per_env * self.env.num_envs
        ...
        if locs['ep_infos']:
            for key in locs['ep_infos'][0]:
                infotensor = torch.tensor([], device=g_dev)
                for ep_info in locs['ep_infos']:
                    ...
                    infotensor = torch.cat((infotensor, ep_info[key].to(g_dev)))
                value = torch.mean(infotensor)
                self.writer.add_scalar('Episode/' + key, value, it)
                ep_string += f"""{f'Mean episode {key}:':>{pad}} {value:.4f}\n"""
        mean_std = self.alg.actor_critic.std.mean()
        fps = int(g_cfg.num_steps_per_env * self.env.num_envs / (locs['collection_time'] + locs['learn_time']))
        ...
        self.writer.add_scalar('Loss/learning_rate', g_cfg.learning_rate, it)
        ...
        string = f" \033[1m Learning iteration {it}/" \
                 f"{self.current_learning_iteration + g_cfg.num_iterations} \033[0m "
